...
class GuiOperatorSafetyTests(unittest.TestCase):
    ...
    def test_rc_zero_requires_matching_completed_metadata(self):
        from widgets.DaqTab import load_completed_run_metadata

        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            raw = root / "sample_run012.dat"
            ...
            self.assertEqual(
                load_completed_run_metadata(metadata, raw, 12), document
            )

            document["acquisition_status"] = "failed"
            metadata.write_text(json.dumps(document), encoding="utf-8")
            with self.assertRaisesRegex(ValueError, "not 'completed'"):
                load_completed_run_metadata(metadata, raw, 12)
