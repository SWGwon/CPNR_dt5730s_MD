...
constexpr std::size_t kMaximumChargeHistogramBins = 150U;
...
bool HasCompleteDcOffsetMetadata(const Json& channel,
                                 const std::string& path) {
  std::size_t extension_fields = 0U;
  // requested_dc_offset/readback_dc_offset were mandatory before this
  // extension. Count only the eight new fields when deciding whether an old
  // sidecar is being parsed.
  for (const char* key : kDcOffsetMetadataKeys) {
    if (channel.contains(key)) ++extension_fields;
  }
  if (extension_fields != 0U && extension_fields != 8U) {
    throw std::runtime_error(
        path + " has a partial DC-offset calibration metadata extension");
  }
  return extension_fields == 8U;
}

bool HasCompleteDcOffsetCalibrationMetadata(const Json& hardware,
                                            const std::string& path) {
  std::size_t fields = 0U;
  for (const char* key : kDcOffsetCalibrationMetadataKeys) {
    if (hardware.contains(key)) ++fields;
  }
  if (fields != 0U && fields != kDcOffsetCalibrationMetadataKeys.size()) {
    throw std::runtime_error(
        path + " has a partial DC-offset calibration settings extension");
  }
  return fields == kDcOffsetCalibrationMetadataKeys.size();
}
...
struct ChannelMetadata {
  ...
  // DC-offset calibration provenance was added as an atomic extension to
  // runtime metadata schema v2.  Keeping the presence bit separate preserves
  // validation of v1 and early-v2 ROOT files while ensuring that a partially
  // written extension can never be mistaken for authenticated calibration.
  bool dc_offset_metadata_available = false;
  std::string dc_offset_mode;
  std::uint32_t requested_dc_offset = 0U;
  std::uint32_t initial_dc_offset_dac = 0U;
  std::uint32_t final_dc_offset_dac = 0U;
  double requested_baseline_percent = 0.0;
  bool requested_baseline_available = false;
  std::uint32_t target_baseline_adc = 0U;
  bool target_baseline_available = false;
  double baseline_error_adc = 0.0;
  bool baseline_error_available = false;
  std::uint32_t dc_offset_adjustment_iterations = 0U;
  bool dc_offset_converged = false;
  bool dc_offset_convergence_available = false;
  std::string mode;
...
struct HardwareMetadata {
  ...
  bool dc_offset_calibration_available = false;
  double dc_offset_target_tolerance_percent = 0.0;
  std::uint32_t dc_offset_max_adjustment_iterations = 0U;
  std::uint32_t dc_offset_dac_busy_timeout_ms = 0U;
  std::uint32_t dc_offset_step_settling_time_ms = 0U;
  std::uint32_t clock_source = 0U;
  std::uint32_t run_sync_mode = 0U;
  std::string polarity;
  std::uint32_t record_mask = 0U;
  std::uint32_t record_length = 0U;
  std::uint32_t record_length_granularity = 8U;
  std::uint32_t post_trigger = 0U;
  std::uint32_t external_mode = 0U;
  std::uint32_t self_mode = 0U;
  std::uint32_t self_mask = 0U;
  // Added after schema v2 was introduced.  Missing fields are the legacy
  // hardware-trigger configuration (software random trigger disabled).
  std::uint32_t software_random_trigger_mode = 0U;
  ...
  std::uint32_t waveform_dsp_schema =
...
void ValidateConfigAgainstMetadata(const std::string& config_contents,
  ...
  try {
    const DAQRecordLengthContract record_length_contract =
        hardware.record_length_granularity ==
    ...
    settings = LoadDAQHardwareSettings(
        ConfigParser::FromText(config_contents, "embedded RunConfigExact"),
  ...
  compare("Digitizer.SelfTriggerMode", settings.self_trigger_mode,
          hardware.self_mode);
  compare("Digitizer.SoftwareRandomTriggerMode",
          settings.software_random_trigger_mode,
          hardware.software_random_trigger_mode);
  if (!MetadataApproxEqual(settings.software_random_trigger_rate_hz,
                           hardware.software_random_trigger_rate_hz)) {
    mismatches.push_back(
        {{"key", "Digitizer.SoftwareRandomTriggerRateHz"},
         {"config", settings.software_random_trigger_rate_hz},
         {"metadata", hardware.software_random_trigger_rate_hz}});
  }
  ...
  const bool config_uses_target_baseline = std::any_of(
  ...
  if (config_uses_target_baseline &&
      !hardware.dc_offset_calibration_available) {
    mismatches.push_back(
        {{"key", "DCOffsetCalibration.metadata_extension"},
         {"config", "required_by_target_baseline"},
         {"metadata", "missing"}});
  }
  if (hardware.dc_offset_calibration_available) {
    if (!MetadataApproxEqual(
            settings.dc_offset_calibration.target_tolerance_percent,
            hardware.dc_offset_target_tolerance_percent)) {
      mismatches.push_back(
          {{"key", "DCOffsetCalibration.TargetTolerancePercent"},
           {"config",
            settings.dc_offset_calibration.target_tolerance_percent},
           {"metadata", hardware.dc_offset_target_tolerance_percent}});
    }
    compare("DCOffsetCalibration.MaxAdjustmentIterations",
            settings.dc_offset_calibration.max_adjustment_iterations,
            hardware.dc_offset_max_adjustment_iterations);
    compare("DCOffsetCalibration.DacBusyTimeoutMs",
            settings.dc_offset_calibration.dac_busy_timeout_ms,
            hardware.dc_offset_dac_busy_timeout_ms);
    compare("DCOffsetCalibration.StepSettlingTimeMs",
            settings.dc_offset_calibration.step_settling_time_ms,
            hardware.dc_offset_step_settling_time_ms);
  }
  ...
  for (int channel = 0; channel < kChannelCount; ++channel) {
    ...
    if (configured.dc_offset_mode == DAQDCOffsetMode::kTargetBaseline) {
      if (!channel_metadata.dc_offset_metadata_available ||
          channel_metadata.dc_offset_mode != "target_baseline_percent") {
        mismatches.push_back(
            {{"key", prefix + "DCOffsetMode"},
             {"config", "TargetBaseline"},
             {"metadata",
              channel_metadata.dc_offset_metadata_available
                  ? Json(channel_metadata.dc_offset_mode)
                  : Json(nullptr)}});
      } else {
        if (!MetadataApproxEqual(configured.baseline_target_percent,
                                 channel_metadata
                                     .requested_baseline_percent)) {
          mismatches.push_back(
              {{"key", prefix + "BaselineTargetPercent"},
               {"config", configured.baseline_target_percent},
               {"metadata",
                channel_metadata.requested_baseline_percent}});
        }
        compare(prefix + "TargetBaselineAdc",
                configured.target_baseline_adc,
                channel_metadata.target_baseline_adc);
        compare(prefix + "InitialDCOffsetDac", configured.dc_offset,
                channel_metadata.initial_dc_offset_dac);
      }
    } else {
      if (channel_metadata.dc_offset_metadata_available &&
          channel_metadata.dc_offset_mode != "raw_dac_code") {
        mismatches.push_back(
            {{"key", prefix + "DCOffsetMode"},
             {"config", "RawDac"},
             {"metadata", channel_metadata.dc_offset_mode}});
      }
      compare(prefix + "DCOffset", configured.dc_offset,
              channel_metadata.final_dc_offset_dac);
    }
  ...
  *config_contents = ReadStringObject(file, "RunConfigExact");
  ...
  try {
    ...
    RequireUnsigned(hardware, "dc_offset_dac_bits", "metadata.hardware", 16U,
                    16U);
    parsed_hardware.dc_offset_calibration_available =
        HasCompleteDcOffsetCalibrationMetadata(hardware,
                                               "metadata.hardware");
    if (parsed_hardware.dc_offset_calibration_available) {
      parsed_hardware.dc_offset_target_tolerance_percent = RequireNumber(
          hardware, "dc_offset_target_tolerance_percent", "metadata.hardware",
          0.0, 10.0);
      parsed_hardware.dc_offset_max_adjustment_iterations =
          static_cast<std::uint32_t>(RequireUnsigned(
              hardware, "dc_offset_max_adjustment_iterations",
              "metadata.hardware", 1U, 32U));
      parsed_hardware.dc_offset_dac_busy_timeout_ms =
          static_cast<std::uint32_t>(RequireUnsigned(
              hardware, "dc_offset_dac_busy_timeout_ms", "metadata.hardware",
              1U, 60000U));
      parsed_hardware.dc_offset_step_settling_time_ms =
          static_cast<std::uint32_t>(RequireUnsigned(
              hardware, "dc_offset_step_settling_time_ms",
              "metadata.hardware", 0U, 600000U));
    }
    if (schema >= 2U) {
      ...
      parsed_hardware.waveform_dsp_schema =
          static_cast<std::uint32_t>(RequireUnsigned(
              hardware, "waveform_dsp_schema", "metadata.hardware",
              cpnr::kWaveformDspSchemaClampedCharge,
              cpnr::kWaveformDspCurrentSchema));
      if (parsed_hardware.waveform_dsp_schema !=
              cpnr::kWaveformDspSchemaClampedCharge &&
          parsed_hardware.waveform_dsp_schema !=
      ...
      if (parsed_hardware.waveform_dsp_schema ==
          cpnr::kWaveformDspSchemaPeakCenteredCharge) {
        if (RequireString(hardware, "dsp_charge_anchor",
                          "metadata.hardware") !=
                cpnr::kPeakCenteredChargeAnchor ||
            RequireUnsigned(hardware, "dsp_charge_window_pre_ns",
                            "metadata.hardware", 0U,
                            std::numeric_limits<std::uint32_t>::max()) !=
                cpnr::kPeakCenteredChargePreNs ||
            RequireUnsigned(hardware, "dsp_charge_window_post_ns",
                            "metadata.hardware", 0U,
                            std::numeric_limits<std::uint32_t>::max()) !=
                cpnr::kPeakCenteredChargePostNs ||
            RequireUnsigned(hardware, "dsp_charge_window_samples",
                            "metadata.hardware", 1U,
                            std::numeric_limits<std::uint32_t>::max()) !=
                cpnr::kPeakCenteredChargeWindowSamples ||
            RequireString(hardware, "dsp_short_charge_semantics",
                          "metadata.hardware") !=
                cpnr::kPeakCenteredShortChargeSemantics ||
            RequireString(hardware, "dsp_pulse_time_semantics",
                          "metadata.hardware") !=
                cpnr::kPeakCenteredPulseTimeSemantics) {
          throw std::runtime_error(
              "metadata.hardware peak-centered charge contract is invalid");
        }
      }
    ...
    if (hardware.contains("record_length_granularity_samples")) {
      if (schema < 2U) {
        throw std::runtime_error(
            "metadata schema v1 cannot declare record-length granularity");
      }
      parsed_hardware.record_length_granularity =
          static_cast<std::uint32_t>(RequireUnsigned(
              hardware, "record_length_granularity_samples",
              "metadata.hardware", 1U, 102400U));
      if (parsed_hardware.record_length_granularity != 8U &&
          parsed_hardware.record_length_granularity !=
              dt5730_constraints::kRecordLengthGranularitySamples) {
        throw std::runtime_error(
            "record-length granularity is unsupported");
      }
    }
    if (parsed_hardware.record_length %
            parsed_hardware.record_length_granularity !=
        0U) {
      throw std::runtime_error(
          "record length does not match its metadata granularity");
    }
    ...
    if (schema >= 2U) {
      const std::uint64_t observed_readback = RequireUnsigned(
          hardware, "post_trigger_readback_percent", "metadata.hardware",
          0U, 100U);
      const std::uint32_t predicted_readback =
          parsed_hardware.record_length_granularity ==
                  dt5730_constraints::
              ...
              : parsed_hardware.post_trigger;
      if (observed_readback != predicted_readback) {
        throw std::runtime_error(
            "post-trigger readback differs from the value required by its "
            "record-length contract");
      }
    }
    ...
    const bool has_random_mode =
        hardware.contains("software_random_trigger_mode");
    const bool has_random_rate =
        hardware.contains("software_random_trigger_rate_hz");
    if (has_random_mode != has_random_rate) {
      throw std::runtime_error(
          "metadata.hardware software-random trigger mode/rate must be "
          "provided together");
    }
    if (has_random_mode) {
      parsed_hardware.software_random_trigger_mode =
          static_cast<std::uint32_t>(RequireUnsigned(
              hardware, "software_random_trigger_mode", "metadata.hardware",
              0U, 1U));
      parsed_hardware.software_random_trigger_rate_hz = RequireNumber(
          hardware, "software_random_trigger_rate_hz", "metadata.hardware",
          0.0, kMaximumSoftwareRandomTriggerRateHz);
      if ((parsed_hardware.software_random_trigger_mode != 0U) !=
          (parsed_hardware.software_random_trigger_rate_hz > 0.0)) {
        throw std::runtime_error(
            "metadata.hardware software-random trigger mode/rate are "
            "inconsistent");
      }
      if (parsed_hardware.software_random_trigger_mode != 0U &&
          parsed_hardware.software_random_trigger_rate_hz <
              kMinimumSoftwareRandomTriggerRateHz) {
        throw std::runtime_error(
            "metadata.hardware software-random trigger rate is below the "
            "supported minimum");
      }
    }
    ...
    const std::uint32_t metadata_pre_trigger_samples =
        parsed_hardware.record_length_granularity ==
                dt5730_constraints::kRecordLengthGranularitySamples
            ? dt5730_constraints::PredictPostTriggerLayout(
                  parsed_hardware.record_length,
            ...
            : static_cast<std::uint32_t>(
                  static_cast<std::uint64_t>(
                      parsed_hardware.record_length) *
                  (100U - parsed_hardware.post_trigger) / 100U);
    if (metadata_pre_trigger_samples <
        dt5730_constraints::kMinimumPreTriggerSamples) {
      throw std::runtime_error(
          "record length/post-trigger leave less than 160 ns pre-trigger");
    }
    const bool software_random_enabled =
        parsed_hardware.software_random_trigger_mode != 0U;
    if (parsed_hardware.external_mode == 0U &&
        parsed_hardware.self_mode == 0U && !software_random_enabled) {
      throw std::runtime_error(
          "external and self trigger modes cannot both be disabled unless "
          "software random trigger is enabled");
    }
    if (software_random_enabled &&
        (parsed_hardware.external_mode != 0U ||
         parsed_hardware.self_mode != 0U || parsed_hardware.self_mask != 0U)) {
      throw std::runtime_error(
          "software random trigger cannot be mixed with external or self "
          "trigger sources");
    }
    ...
    const std::uint32_t expected_global =
        ...
        ((software_random_enabled || !parsed_hardware.explicit_routing)
             ? (1U << 31U)
             : 0U);
    ...
    for (const Json& channel : channels) {
      ...
      if (RequireString(channel, "polarity_readback",
                        "metadata.channels[]") != parsed_hardware.polarity) {
      ...
      parsed_channel.dc_offset_metadata_available =
          HasCompleteDcOffsetMetadata(channel, "metadata.channels[]");
      if (parsed_channel.dc_offset_metadata_available !=
          parsed_hardware.dc_offset_calibration_available) {
        throw std::runtime_error(
            "hardware/channel DC-offset metadata extensions are inconsistent");
      }
      if (dc_offset_extension_present.has_value() &&
          *dc_offset_extension_present !=
              parsed_channel.dc_offset_metadata_available) {
        throw std::runtime_error(
            "metadata channels mix legacy and extended DC-offset schemas");
      }
      dc_offset_extension_present =
          parsed_channel.dc_offset_metadata_available;
      if (!parsed_channel.dc_offset_metadata_available) {
        if (requested_offset != readback_offset) {
          throw std::runtime_error(
              "legacy channel DC-offset write/readback mismatch");
        }
        parsed_channel.initial_dc_offset_dac =
            static_cast<std::uint32_t>(requested_offset);
        parsed_channel.final_dc_offset_dac =
            static_cast<std::uint32_t>(readback_offset);
      } else {
        parsed_channel.dc_offset_mode = RequireString(
            channel, "dc_offset_mode", "metadata.channels[]");
        parsed_channel.initial_dc_offset_dac =
            static_cast<std::uint32_t>(RequireUnsigned(
                channel, "initial_dc_offset_dac", "metadata.channels[]", 0U,
                65535U));
        parsed_channel.final_dc_offset_dac =
            static_cast<std::uint32_t>(RequireUnsigned(
                channel, "final_dc_offset_dac", "metadata.channels[]", 0U,
                65535U));
        parsed_channel.dc_offset_adjustment_iterations =
            static_cast<std::uint32_t>(RequireUnsigned(
                channel, "dc_offset_adjustment_iterations",
                "metadata.channels[]", 0U,
                std::numeric_limits<std::uint32_t>::max()));
        if (requested_offset != parsed_channel.final_dc_offset_dac ||
            readback_offset != parsed_channel.final_dc_offset_dac) {
          throw std::runtime_error(
              "DC-offset write/readback aliases differ from final DAC "
              "metadata");
        }

        if (parsed_channel.dc_offset_mode == "raw_dac_code") {
          RequireNull(channel, "requested_baseline_percent",
                      "metadata.channels[]");
          RequireNull(channel, "target_baseline_adc", "metadata.channels[]");
          RequireNull(channel, "baseline_error_adc", "metadata.channels[]");
          RequireNull(channel, "dc_offset_converged", "metadata.channels[]");
          if (parsed_channel.initial_dc_offset_dac !=
                  parsed_channel.final_dc_offset_dac ||
              parsed_channel.dc_offset_adjustment_iterations != 0U) {
            throw std::runtime_error(
                "raw DAC mode must not adjust the configured DC-offset code");
          }
        } else if (parsed_channel.dc_offset_mode ==
                   "target_baseline_percent") {
          parsed_channel.requested_baseline_percent = RequireNumber(
              channel, "requested_baseline_percent", "metadata.channels[]",
              5.0, 95.0);
          parsed_channel.requested_baseline_available = true;
          parsed_channel.target_baseline_adc =
              static_cast<std::uint32_t>(RequireUnsigned(
                  channel, "target_baseline_adc", "metadata.channels[]", 0U,
                  static_cast<std::uint64_t>(kAdcMaximum)));
          parsed_channel.target_baseline_available = true;
          parsed_channel.baseline_error_adc = RequireNumber(
              channel, "baseline_error_adc", "metadata.channels[]",
              -kAdcMaximum, kAdcMaximum);
          parsed_channel.baseline_error_available = true;
          parsed_channel.dc_offset_converged = RequireBool(
              channel, "dc_offset_converged", "metadata.channels[]");
          parsed_channel.dc_offset_convergence_available = true;
          const auto expected_target = static_cast<std::uint32_t>(std::llround(
              parsed_channel.requested_baseline_percent * kAdcMaximum /
              100.0));
          const double allowed_error =
              parsed_hardware.dc_offset_target_tolerance_percent *
              kAdcMaximum / 100.0;
          if (parsed_channel.target_baseline_adc != expected_target ||
              !parsed_channel.dc_offset_converged ||
              parsed_channel.dc_offset_adjustment_iterations >
                  2U * parsed_hardware.dc_offset_max_adjustment_iterations ||
              std::abs(parsed_channel.baseline_error_adc) >
                  allowed_error + 1e-9) {
            throw std::runtime_error(
                "target-baseline DC-offset metadata is not converged or its "
                "target/error is inconsistent");
          }
        } else {
          throw std::runtime_error("unknown channel DC-offset mode");
        }
      }
      ...
      if (parsed_channel.dc_offset_mode == "target_baseline_percent") {
        if (!parsed_channel.baseline_available ||
            std::abs(parsed_channel.baseline_error_adc -
                     (parsed_channel.measured_baseline -
                      static_cast<double>(
                          parsed_channel.target_baseline_adc))) > 1e-6) {
          throw std::runtime_error(
              "target-baseline measured value/error metadata is inconsistent");
        }
      }
...
Json BuildChargeHistogram(const ChannelAccumulator& accumulator, int channel,
                          std::uint64_t sample_stride,
                          bool partial_event_scan, bool scan_cancelled) {
  const bool stride_sampled = sample_stride > 1U;
  const std::string coverage =
      scan_cancelled
          ? "cancelled_prefix"
      : partial_event_scan
          ? stride_sampled ? "stride_sampled_prefix" : "prefix"
      : stride_sampled ? "stride_sampled_full_scan" : "full_scan";
  Json histogram =
      {{"available", false},
       {"source_branch", "Charge_CH" + std::to_string(channel)},
       {"unit", "ADC.sample"},
       {"binning", "linear"},
       {"bin_edges", Json::array()},
       {"counts", Json::array()},
       {"values_sampled", accumulator.charge_samples.size()},
       {"events_scanned", accumulator.active_events},
       {"sample_stride", sample_stride},
       {"sampled", stride_sampled},
       {"coverage", coverage}};

  if (accumulator.charge_samples.empty() ||
      !std::isfinite(accumulator.charge_min) ||
      !std::isfinite(accumulator.charge_max)) {
    return histogram;
  }

  double lower = accumulator.charge_min;
  double upper = accumulator.charge_max;
  std::size_t bin_count = 1U;
  if (upper > lower) {
    bin_count = std::min<std::size_t>(
        kMaximumChargeHistogramBins,
        std::max<std::size_t>(
            1U, static_cast<std::size_t>(std::ceil(std::sqrt(
                    static_cast<double>(accumulator.charge_samples.size()))))));
  } else {
    const double padding = std::max(0.5, std::abs(lower) * 1.0e-9);
    lower -= padding;
    upper += padding;
  }

  const double bin_width =
      (upper - lower) / static_cast<double>(bin_count);
  if (!(bin_width > 0.0) || !std::isfinite(bin_width)) {
    return histogram;
  }

  std::vector<std::uint64_t> counts(bin_count, 0U);
  for (const double value : accumulator.charge_samples) {
    std::size_t index = bin_count - 1U;
    if (value < upper) {
      const double position = (value - lower) / bin_width;
      if (position <= 0.0) {
        index = 0U;
      } else {
        index = std::min<std::size_t>(
            bin_count - 1U, static_cast<std::size_t>(position));
      }
    }
    ++counts[index];
  }
  ...
  for (std::size_t index = 0U; index <= bin_count; ++index) {
    edges.push_back(index == bin_count
                        ? upper
                        : lower + static_cast<double>(index) * bin_width);
  }
  histogram["available"] = true;
  histogram["bin_edges"] = std::move(edges);
...
Json ValidateRootFile(const std::string& input_path,
  ...
  const bool supported_waveform_dsp_schema =
      waveform_dsp_schema &&
      (*waveform_dsp_schema == cpnr::kWaveformDspSchemaClampedCharge ||
       *waveform_dsp_schema == cpnr::kWaveformDspSchemaSignedCharge ||
       *waveform_dsp_schema ==
           cpnr::kWaveformDspSchemaPeakCenteredCharge);
  const bool requires_explicit_waveform_dsp_schema =
      metadata.parsed && metadata.schema_version >= 2U;
  const bool waveform_dsp_schema_matches_metadata =
      !requires_explicit_waveform_dsp_schema ||
      (supported_waveform_dsp_schema && metadata.hardware.valid &&
       *waveform_dsp_schema ==
           static_cast<int>(metadata.hardware.waveform_dsp_schema));
  // A valid v2 run promises an explicit DSP layout. Continue scanning that
  // layout even when its marker is defective so the report exposes all
  // independent branch/parameter defects, while the contract itself fails.
  const bool scan_explicit_waveform_dsp =
      supported_waveform_dsp_schema ||
      requires_explicit_waveform_dsp_schema;
  const int effective_waveform_dsp_schema =
      supported_waveform_dsp_schema
          ? *waveform_dsp_schema
      : requires_explicit_waveform_dsp_schema && metadata.hardware.valid
          ? static_cast<int>(metadata.hardware.waveform_dsp_schema)
          : cpnr::kWaveformDspSchemaClampedCharge;
    ...
    try {
      const DAQRecordLengthContract record_length_contract =
          metadata.hardware.valid
              ? metadata.hardware.record_length_granularity ==
                        dt5730_constraints::
                            kRecordLengthGranularitySamples
                    ? DAQRecordLengthContract::kCurrentX730
                    : DAQRecordLengthContract::kLegacyMultipleOf8
              : DAQRecordLengthContract::kLegacyOrCurrent;
      embedded_settings = LoadDAQHardwareSettings(
          ConfigParser::FromText(*config_contents,
                                 "embedded RunConfigExact"),
          record_length_contract,
          effective_waveform_dsp_schema ==
                  static_cast<int>(
                      cpnr::kWaveformDspSchemaPeakCenteredCharge)
              ? DAQWaveformDspContract::kPeakCentered
              : DAQWaveformDspContract::kLegacyThresholdGates);
      if (scan_explicit_waveform_dsp) {
  ...
  waveform_dsp_settings.preserve_signed_charge =
      effective_waveform_dsp_schema !=
      cpnr::kWaveformDspSchemaClampedCharge;
  waveform_dsp_settings.integration_mode =
      effective_waveform_dsp_schema ==
              cpnr::kWaveformDspSchemaPeakCenteredCharge
  ...
  } else if (requires_explicit_waveform_dsp_schema &&
             !waveform_dsp_schema_matches_metadata) {
    raw_fidelity_failure =
        "Schema-v2 provenance requires matching supported metadata and ROOT "
        "WaveformDspSchema values before RAW-to-ROOT fidelity can be trusted";
  } else {
    try {
      ...
      settings.compare_short_charge = scan_explicit_waveform_dsp;
  ...
  if (supported_waveform_dsp_schema) {
    if (requires_explicit_waveform_dsp_schema &&
        !waveform_dsp_schema_matches_metadata) {
      waveform_dsp_errors.push_back(
          {{"name", "WaveformDspSchema"},
           {"observed", *waveform_dsp_schema},
           {"expected", metadata.hardware.valid
                            ? Json(metadata.hardware.waveform_dsp_schema)
                            : Json(nullptr)}});
    }
    if (!embedded_settings) {
      ...
      if (*waveform_dsp_schema ==
          cpnr::kWaveformDspSchemaPeakCenteredCharge) {
        require_dsp_int("DspChargeWindowPreNs",
                        cpnr::kPeakCenteredChargePreNs);
        require_dsp_int("DspChargeWindowPostNs",
                        cpnr::kPeakCenteredChargePostNs);
        require_dsp_int("DspChargeWindowSamples",
                        cpnr::kPeakCenteredChargeWindowSamples);
        const auto require_dsp_string =
            [&](const char* name, const char* expected) {
              const std::optional<std::string> observed =
                  ReadStringObject(*file, name);
        ...
        require_dsp_string("DspChargeAnchor",
                           cpnr::kPeakCenteredChargeAnchor);
        require_dsp_string("DspShortChargeSemantics",
                           cpnr::kPeakCenteredShortChargeSemantics);
        require_dsp_string("DspPulseTimeSemantics",
                           cpnr::kPeakCenteredPulseTimeSemantics);
      }
    }
    ...
    checks.Add("FAIL", "schema", "waveform_dsp_contract",
               waveform_dsp_schema ? Json(*waveform_dsp_schema)
                                   : Json(nullptr),
               Json::array({cpnr::kWaveformDspSchemaClampedCharge,
                            cpnr::kWaveformDspSchemaSignedCharge,
                            cpnr::kWaveformDspSchemaPeakCenteredCharge}),
               "WaveformDspSchema exists but is not the supported schema version/type.");
  } else if (requires_explicit_waveform_dsp_schema) {
    checks.Add(
        "FAIL", "schema", "waveform_dsp_contract", nullptr,
        metadata.hardware.valid ? Json(metadata.hardware.waveform_dsp_schema)
                                : Json(cpnr::kWaveformDspCurrentSchema),
        "Schema-v2 runtime provenance requires a matching supported "
        "WaveformDspSchema marker; it is missing, so scalar DSP semantics "
        "cannot be authenticated.");
  } else {
    checks.Add(
        "WARN", "compatibility", "legacy_waveform_dsp_contract",
        "hard-coded adaptive baseline and end-of-record integration",
        "WaveformDspSchema=1, 2, or 3 with authenticated DSP settings",
  ...
  report["summary"]["waveform_dsp_schema"] =
      waveform_dsp_schema ? Json(*waveform_dsp_schema) : Json(nullptr);
    ...
    for (std::uint64_t entry = 0U; entry < scan_limit; ++entry) {
      ...
      if (has_audit_branches) {
        ...
        if (board_event_counter > 0xFFFFFFU) {
          ...
          previous_board_counter_available = false;
        } else {
          if (previous_board_counter_available) {
          ...
          previous_board_counter_available = true;
        }
      }
      const bool record_length_matches_known_contract =
          metadata.hardware.valid
              ? record_length %
                        metadata.hardware.record_length_granularity ==
                    0U
      ...
      const bool shape_is_valid =
          ...
          record_length_matches_known_contract;
      if (!shape_is_valid ||
          (metadata.hardware.valid &&
      ...
      for (int channel = 0; channel < kChannelCount; ++channel) {
        ...
        const bool peak_centered_charge =
            effective_waveform_dsp_schema ==
            cpnr::kWaveformDspSchemaPeakCenteredCharge;
        const std::size_t maximum_charge_samples =
            peak_centered_charge
                ? std::min<std::size_t>(record_length,
                                        cpnr::kPeakCenteredChargeWindowSamples)
                : scan_explicit_waveform_dsp
                      ? waveform_dsp_settings.long_gate_samples
                      : record_length;
        const std::size_t maximum_short_charge_samples =
            peak_centered_charge
                ? maximum_charge_samples
                : waveform_dsp_settings.short_gate_samples;
        const double theoretical_charge_max =
            static_cast<double>(maximum_charge_samples) * kAdcMaximum;
        const double theoretical_short_charge_max =
            static_cast<double>(maximum_short_charge_samples) * kAdcMaximum;
        const double theoretical_charge_min =
            waveform_dsp_settings.preserve_signed_charge
                ? -theoretical_charge_max
                : 0.0;
        const double theoretical_short_charge_min =
            waveform_dsp_settings.preserve_signed_charge
                ? -theoretical_short_charge_max
                : 0.0;
        ...
        if (baseline[channel] < 0.0 || baseline[channel] > kAdcMaximum ||
            ...
            (scan_explicit_waveform_dsp &&
             (short_charge[channel] <
                  theoretical_short_charge_min - 1e-9 ||
              short_charge[channel] > theoretical_short_charge_max + 1e-9)) ||
            charge[channel] < theoretical_charge_min - 1e-9 ||
        ...
        if ((entry % sample_stride) == 0U) {
          accumulator.charge_samples.push_back(charge[channel]);
  ...
  if (tree != nullptr && scalar_schema_valid && scan_limit != 0U) {
    ...
    if (!metadata.present &&
      ...
      checks.Add("WARN", "compatibility", "legacy_record_length_granularity",
                 observed_lengths, "current files use a multiple of 10",
  ...
  // Construct these optionals in a known disengaged state, then assign only
  // in the schema where each object is valid.  Some GCC optimization passes
  // lose the relationship between a conditionally constructed optional's
  // engagement flag and its scalar payload, producing false
  // -Wmaybe-uninitialized diagnostics at guarded dereferences below.
  std::optional<double> live_time;
  std::optional<double> dead_time_percent;
  std::optional<double> ttt_raw_lsb_ns;
  std::optional<double> ttt_resolution_ns;
  std::optional<double> adc_sample_period_ns;
  std::optional<double> recorded_window_sum;
  std::optional<double> recorded_window_percent;
  if (current_timing_schema) {
    ttt_raw_lsb_ns =
        ReadParameter<double>(*file, "TriggerTimeTagRawLsb_ns");
    ttt_resolution_ns = ReadParameter<double>(
        *file, "TriggerTimeTagObservableResolution_ns");
    ...
    live_time = ReadParameter<double>(*file, "LiveTime_sec");
    dead_time_percent = ReadParameter<double>(*file, "DeadTime_pct");
  }
  ...
  for (int channel = 0; channel < kChannelCount; ++channel) {
    ...
    Json dc_offset_json =
        {{"metadata_extension_available",
          threshold.present
              ? Json(threshold.dc_offset_metadata_available)
              : Json(nullptr)},
         {"mode",
          threshold.dc_offset_metadata_available
              ? Json(threshold.dc_offset_mode)
              : threshold.present ? Json("legacy_raw_dac_code")
                                  : Json(nullptr)},
         {"requested_baseline_percent",
          NullOrDouble(threshold.requested_baseline_available,
                       threshold.requested_baseline_percent)},
         {"target_baseline_adc",
          NullOrUnsigned(threshold.target_baseline_available,
                         threshold.target_baseline_adc)},
         {"initial_dac",
          threshold.present ? Json(threshold.initial_dc_offset_dac)
                            : Json(nullptr)},
         {"final_dac",
          threshold.present ? Json(threshold.final_dc_offset_dac)
                            : Json(nullptr)},
         {"measured_baseline_adc",
          NullOrDouble(threshold.baseline_available,
                       threshold.measured_baseline)},
         {"baseline_error_adc",
          NullOrDouble(threshold.baseline_error_available,
                       threshold.baseline_error_adc)},
         {"adjustment_iterations",
          threshold.dc_offset_metadata_available
              ? Json(threshold.dc_offset_adjustment_iterations)
              : Json(nullptr)},
         {"converged",
          threshold.dc_offset_convergence_available
              ? Json(threshold.dc_offset_converged)
              : Json(nullptr)}};
    Json metrics =
        {{"events_scanned", accumulator.active_events},
         ...
         {"actual_dc_offset_target_error_adc", nullptr},
    ...
    if (metadata.hardware.valid && active && tail_available &&
        threshold.dc_offset_metadata_available &&
        threshold.dc_offset_mode == "target_baseline_percent" &&
        threshold.target_baseline_available) {
      const double target_error =
          tail_median - static_cast<double>(threshold.target_baseline_adc);
      const double allowed_error =
          metadata.hardware.dc_offset_target_tolerance_percent *
          kAdcMaximum / 100.0;
      metrics["actual_dc_offset_target_error_adc"] = target_error;
      const bool within_target = std::abs(target_error) <= allowed_error + 1e-9;
      const bool positive_target_unproven =
          within_target && (partial_event_scan || metric_subsampled);
      checks.Add(!within_target
                     ? "FAIL"
                 : positive_target_unproven ? "SKIP"
                                            : "PASS",
                 "data_quality",
                 "channel_" + std::to_string(channel) + "_dc_offset_target",
                 {{"stable_baseline_adc", tail_median},
                  {"target_baseline_adc", threshold.target_baseline_adc},
                  {"actual_error_adc", target_error},
                  {"calibration_baseline_adc",
                   threshold.measured_baseline},
                  {"calibration_error_adc", threshold.baseline_error_adc},
                  {"final_dac", threshold.final_dc_offset_dac}},
                 {{"maximum_absolute_error_adc", allowed_error},
                  {"requested_baseline_percent",
                   threshold.requested_baseline_percent}},
                 !within_target
                     ? "The event-file baseline moved outside the authenticated DC-offset target tolerance."
                 : positive_target_unproven
                     ? "The scanned baseline matches the target, but sampled coverage cannot establish the complete run."
                     : "The stable event-file baseline remains within the authenticated target tolerance.");
    }
    ...
    report["channels"].push_back(
        {{"channel", channel},
         ...
         {"dc_offset", std::move(dc_offset_json)},
         {"threshold", std::move(threshold_json)},
         {"metrics", std::move(metrics)},
         {"charge_histogram",
          BuildChargeHistogram(accumulator, channel, sample_stride,
                               partial_event_scan, scan_cancelled)}});
  }
